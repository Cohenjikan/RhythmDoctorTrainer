# RD Trainer (节奏医生修改器) — uninstaller
$ErrorActionPreference = 'Stop'
try { [Console]::OutputEncoding = [System.Text.Encoding]::UTF8 } catch {}

Write-Host ""
Write-Host "==== 节奏医生修改器  卸载 ====" -ForegroundColor Cyan
Write-Host ""
...
if (-not (Test-Path (Join-Path $game 'Rhythm Doctor.exe'))) { Write-Host "[错误] 目录无效。" -ForegroundColor Red; exit 1 }
