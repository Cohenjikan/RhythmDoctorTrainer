...
> Applies on level **start / restart** — there is **no mid-song change** (the engine fixes BPM and pitch at load, so a live change would desync). A "restart and apply" button is provided in the menu.

### Widened judgment window · No-fail · QoL
...
### Jump to any level (bypass story / NPC gating)
...
### Developer / debug mode + achievement controls
